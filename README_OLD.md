...
The simulation will output results to `sim_output.json`.

## Configuration

All hyperparameters are defined in `hyperparameters.txt` in the project root. You can edit this file to adjust simulation behavior:

- **Input Computation**: Wall pressure, crowd radius, minimum distance
- **Output Thresholds**: Apoptosis threshold, edge pruning, growth multipliers
- **Snap Logic**: Angle thresholds, sprout conditions, anastomosis radius
- **Shift Logic**: Movement rate, wall avoidance, unstuck forces
- **Collision Detection**: Raycast step sizes, safety margins

Edit `hyperparameters.txt` and re-run the simulation to see the effects. No recompilation needed.
